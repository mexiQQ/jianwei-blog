// Shared inline icons + PaperLinks row for publication/preprint entries.
...
export const PaperLinks = ({ pdf, code, site, inline = false }) => (
  <span className={(inline ? "inline-flex" : "mt-1 flex") + " flex-wrap items-center gap-x-3 gap-y-1 text-sm"}>
    ...
    {code && (
      <a href={code} target="_blank" rel="noopener noreferrer" className={linkClass}>
        <CodeIcon /> <span>Code</span>
      </a>
    )}
    {site && (
      <a href={site} target="_blank" rel="noopener noreferrer" className={linkClass}>
        <SiteIcon /> <span>Project page</span>
      </a>
    )}
  </span>
);
